...
const skillCategories = [
  ...
  { name: "Backend", skills: ["Node.js", "Python", "Rust", "PostgreSQL", "Redis", "REST APIs", "GraphQL"] },
  { name: "AI / ML", skills: ["LLM Integration", "AI Agents", "MCP Protocol", "Prompt Engineering", "LangChain", "OpenAI / Claude / Gemini APIs", "LLM Evaluation"] },
  { name: "Tools & Infrastructure", skills: ["Git", "Docker", "Linux", "CI/CD", "PyPI Publishing", "Cloudflare", "Vercel"] },
];
    ...
    <>
      ...
      <section className="mx-auto max-w-content px-6 pb-16 md:px-10 md:pb-20">
        <div className="grid gap-6 md:grid-cols-2">
          {skillCategories.map((cat, i) => (
            <Inview key={cat.name} from={{ opacity: 0, y: 20 }} to={{ opacity: 1, y: 0 }} mode="once" delayIn={i * 100} config={{ tension: 100, friction: 20 }}>
              <div className="card p-6">
                <h3 className="text-sm font-medium uppercase tracking-widest text-brand">{cat.name}</h3>
      ...
      <section className="shadow-border">
        <div className="mx-auto max-w-content px-6 py-16 md:px-10 md:py-20">
          <Inview from={{ opacity: 0, y: 20 }} to={{ opacity: 1, y: 0 }} mode="once">
            <h2 className="text-2xl font-medium tracking-heading" style={{ fontFeatureSettings: "'liga' 1" }}>Languages</h2>
          </Inview>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {languages.map((lang, i) => (
              <Inview key={lang.name} from={{ opacity: 0, y: 20 }} to={{ opacity: 1, y: 0 }} mode="once" delayIn={i * 100}>
                <div className="card p-4">
                  <div className="text-sm font-medium tracking-tight" style={{ fontFeatureSettings: "'liga' 1" }}>{lang.name}</div>
